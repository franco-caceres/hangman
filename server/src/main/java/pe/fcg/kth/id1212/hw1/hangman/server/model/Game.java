...
import pe.fcg.kth.id1212.hw1.hangman.server.util.StringUtils;
...
class Game {
    ...
    Result makeGuess(Guess guess) {
        if(status != Status.PLAYING) {
            throw new IllegalStateException("Game has " + status + " status.");
        }
        if(guess.getContent() == null || guess.getContent().length() == 0) {
            throw new IllegalArgumentException("Empty guess found.");
        }
